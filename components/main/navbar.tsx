    ...
    <div className="w-full h-[65px] fixed top-0 shadow-lg shadow-[#2A0E61]/50 bg-[#03001427] backdrop-blur-md z-50 px-4 md:px-10">
      ...
      <div className="w-full h-full flex items-center justify-between m-auto">
        ...
        >
          ...
          />
          <div className="hidden md:flex font-bold ml-[10px] text-gray-300">Rai Yashmit</div>
        </Link>
          ...
          <div className="flex items-center justify-between w-full h-auto border-[rgba(112,66,248,0.38)] bg-[rgba(3,0,20,0.37)] mr-[15px] px-[20px] py-[10px] rounded-full text-gray-200">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.title}
                href={link.link}
                className="cursor-pointer hover:text-[rgb(112,66,248)] transition text-[14px]"
              >
                {link.title}
              </Link>
            ))}
        ...
        {/* Hamburger Menu */}
        <button
          className="md:hidden text-gray-300 focus:outline-none text-3xl"
